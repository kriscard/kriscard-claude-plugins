...
description: Restores previously saved task context. Use when user says "/context-restore", wants to resume a task, needs to restore context, or is returning to previous work.
model: sonnet
...
If mismatch:
```
Context was saved on branch: feat/purchase-modal
...
```
Some files have changed since context was saved:
